  ...
  const faqs = [
    {
      ...
      a: "Yes, you can upgrade your plan at any time. The changes will be reflected in the remainder of your billing cycle, and you'll be charged a prorated amount for the upgrade.",
    },
    {
      q: "Do you offer refunds?",
      a: "We offer a 14-day money-back guarantee. If you're not satisfied with our service, you can request a full refund within 14 days of purchase.",
    },
    {
      q: "What payment methods do you accept?",
      a: "We accept all major credit cards including Visa, Mastercard, and American Express. We also support payment via PayPal for your convenience.",
    },
    {
    ...
    },
    {
      ...
      a: "When you reach your app limit, you'll need to upgrade to a higher tier to add more apps. Alternatively, you can remove an existing app to make room for a new one.",
    },
    {
      q: "Do you offer annual billing?",
      a: "Yes, we offer annual billing with a discount. Contact our sales team for more information about annual plans and enterprise pricing.",
    },
  ];
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        ...
        <header className="mb-10 text-center">
          <div>
            ...
            <p className="mt-2 text-base text-white/60 max-w-3xl mx-auto">
              Here are some common questions and answers that you might encounter when using Insightify. If you don't find the answer you're looking for, feel free to reach out.
            </p>
          </div>
        </header>
        ...
        <section className="relative max-w-4xl mx-auto">
          <div className="space-y-0">
            {filtered.map((item, i) => (
              <FAQItem key={i} q={item.q} a={item.a} index={i + 1} isLast={i === filtered.length - 1} />
            ))}
          </div>
        </section>

        {/* Footer */}
        <footer className="mt-8 border-t border-white/10 pt-6 text-xs text-white/50">
          © {new Date().getFullYear()} Insightify — Transform app reviews into actionable insights.
        </footer>
      </div>
...
function FAQItem({ q, a, index, isLast }) {
  const [open, setOpen] = useState(false);
  return (
    <div className={`border-t border-white/10 ${isLast ? 'border-b' : ''}`}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between text-left py-6 px-2 hover:bg-white/5 transition-colors"
        aria-expanded={open}
      >
        <h3 className="text-base md:text-lg font-semibold leading-tight pr-8">{q}</h3>
        <span className="ml-4 text-xl text-white/60 transition transform flex-shrink-0" style={{ transform: open ? 'rotate(180deg)' : 'rotate(0deg)' }}>
          ▼
        </span>
      </button>
      <div
        className={`grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(.4,0,.2,1)] ${open ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}
      >
        <div className="min-h-0 overflow-hidden">
          <p className="text-sm md:text-base text-white/60 pb-6 px-2">{a}</p>
        </div>
      </div>
    </div>
  );
