...
const OnBoarding2 = () => {
  ...
  // State to track which role is selected
  ...
  const handleContinue = () => {
    if (selectedRole) {
      // Logic to save the role would go here
      console.log("Selected Role:", selectedRole);
      
      // Navigate to next step (or dashboard)
      // For now, let's assume there is a step 3 or go to dashboard
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      {/* Background Decor (Subtle Gradients to match Page 1 vibe) */}
      <div className="absolute top-0 left-0 w-full h-1/2 bg-blue-900/10 blur-[120px] rounded-full pointer-events-none"></div>
      <div className="absolute bottom-0 right-0 w-full h-1/2 bg-purple-900/10 blur-[120px] rounded-full pointer-events-none"></div>
      ...
      <div className="w-full max-w-4xl z-10">
            ...
            {roles.map((role) => (
                <div 
                    ...
                    className={`
                        ...
                        ${selectedRole === role.id 
                            ? "bg-zinc-900 border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.15)] ring-1 ring-blue-500/50" // Active State
                ...
                >
                    ...
                    <div className={`
                        ...
                        ${selectedRole === role.id ? "bg-blue-600 text-white" : "bg-zinc-800 text-zinc-400 group-hover:text-white group-hover:bg-zinc-700"}
                    `}>
                        {role.icon}
                    </div>
                    ...
                    {/* Text */}
                    <div>
                        <h3 className={`text-lg font-semibold mb-2 transition-colors ${selectedRole === role.id ? "text-white" : "text-zinc-200"}`}>
                            {role.title}
                        </h3>
        ...
        <div className="flex items-center justify-between max-w-lg mx-auto mt-8">
            ...
            >
                Continue
                <ArrowRight size={16} className={selectedRole ? "opacity-100" : "opacity-0"} />
            </button>
        </div>
